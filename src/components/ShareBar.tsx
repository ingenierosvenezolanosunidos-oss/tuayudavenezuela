...
function buildShare(report: Report, tipoLabel: string) {
  ...
  const text = `${tipoLabel}: ${report.nombre} — vía tuAyudaVenezuela`
  return { maps, link, text }
}

export default function ShareBar({ report }: Props) {
  const { t } = useTranslation()
  const tipoLabel = t(`layers.${LAYER_BY_ID[report.tipo].id}.label`)
  const { maps, link, text } = buildShare(report, tipoLabel)
  ...
  return (
    <div className="mt-4">
      <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400">
        {t('share_bar.section_label')}
      </div>
      <div className="flex items-center gap-2.5">
        <Circle href={wa} label={t('share_bar.whatsapp')} bg="#25D366">
        ...
        <Circle href={x} label={t('share_bar.twitter')} bg="#000000">
          <span className="text-base font-bold leading-none text-white">𝕏</span>
        </Circle>
        <Circle href={fb} label={t('share_bar.facebook')} bg="#1877F2">
          <span className="text-lg font-bold leading-none text-white">f</span>
        </Circle>
        <button
          onClick={instagram}
          aria-label={t('share_bar.instagram')}
